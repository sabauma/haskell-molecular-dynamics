...
module MolecularDynamics.KdTree where
...
fromListWithDepth points depth = node
    where axis = depth `mod` dimension (head points)
          ...
          (left, p:right) = splitAt medianIndex sortedPoints
...
-- fromVectorWithDepth :: (Point a) => Vector a -> Int -> KdTree a
fromVectorWithDepth :: (Vector v a, Point a) => v a -> Int -> KdTree a
fromVectorWithDepth pts depth = runST (V.thaw pts >>= node depth)
    where node :: (M.MVector v p, Point p) => Int -> v (PrimState (ST s)) p -> ST s (KdTree p)
          node depth points
              | M.null points = return KdEmpty
              | otherwise     = do
                  -- Sorted points
                  first <- points `M.read` 0
                  let axis = depth `mod` dimension first
                  sortBy (\ a b -> coord axis a `compare` coord axis b) points

                  let medianIndex = M.length points `div` 2

                  point <- points `M.read` medianIndex

                  left  <- node (depth + 1) (M.take medianIndex points)
                  right <- node (depth + 1) (M.drop (medianIndex + 1) points)

                  return KdNode { kdLeft  = left
                                , kdPoint = point
                                , kdRight = right
                                , kdAxis  = axis }
{-# INLINE fromVectorWithDepth #-}
